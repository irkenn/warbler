    ...
    if form.validate_on_submit():
        if User.authenticate(user.username,
            ...
            try:
                ...
                db.session.rollback() #this is very important otherwise it will break the code!!!!
...
def process_like(message_id):
    """Creates a like for every POST request"""

    if not g.user:
        flash("Unauthorized to retrieve likes, please log in", "danger")

    current_like = Likes.query.filter(Likes.user_id == g.user.id, Likes.message_id == message_id).first()
    if current_like:
...
def get_likes(user_id):
    """Renders all the likes from the user """
